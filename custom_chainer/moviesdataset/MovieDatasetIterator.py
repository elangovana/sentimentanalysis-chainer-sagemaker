...
class MovieDatasetIterator(chainer.dataset.iterator.Iterator):
    ...
    def _getline(self, handle, idx):
        ...
        i = 0
        line = ""
        self.logger.debug("Reading line {} from file {}".format(idx, self.filepath))
        while i <= idx:
            line = next(csv_reader)
            i = i + 1
        self.logger.debug("Read complete line {} from file {}".format(idx, self.filepath))
        return line
    ...
    def getcount(self):
        ...
        # Get file length
        with io.open(self.filepath, encoding=self.encoding) as handle:
            self.logger.debug("Getting count for file {}".format(self.filepath))
            csv_reader = self._get_reader(handle)
            # Skip first line if header
