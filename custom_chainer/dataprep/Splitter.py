...
class Splitter:
    ...
    def split_traintest(self,  first_size_fraction=.8, seed=1572) -> tuple(
    ...
    Splits a file into 2  sets such as training & test.
        ...
        if os.path.isfile( self.file_or_dir):
            dataset = dataprep.YelpChainerDataset.YelpChainerDataset(self.file_or_dir, delimiter=self.delimiter, encoding=self.encoding,
                                                                     quote_charcter=self.quote_character, has_header=self.has_header)
        else:
            dataset = chainer.datasets.ConcatenatedDataset(*self._get_dataset_array(self.file_or_dir))
    ...
    def _get_dataset_array(self, base_dir):
    ...
    def split(self, outputdir,  no_of_parts=None):
        if not os.path.isfile(self.file_or_dir):
            raise ValueError("The constructor argument file_or_dir {} must be a file to invoke this operation.".format(self.file_or_dir))
